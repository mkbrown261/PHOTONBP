...
async def handle_working_tools() -> list[types.TextContent]:  # noqa: C901
    ...
    SCRIPT = (
        ...
        "\n"
        "def sg(name):\n"
        "    try: return getattr(unreal,name,None)\n"
        "    except: return None\n"
        "\n"
        "eal=sg('EditorAssetLibrary')\n"
        "ath=sg('AssetToolsHelpers')\n"
        "sl=sg('SystemLibrary')\n"
        "pa=sg('Paths')\n"
        "chk('core',[\n"
        ...
        "])\n"
        "\n"
        "pbp=sg('PhotonBPLibrary')\n"
        "bel=sg('BlueprintEditorLibrary')\n"
        "chk('blueprint',[\n"
        "  ('BlueprintFactory',sg('BlueprintFactory') is not None),\n"
        ...
        "  ('PBP.add_component',hasattr(pbp,'add_component') if pbp else False),\n"
        "  ('PBP.add_interface',hasattr(pbp,'add_interface') if pbp else False),\n"
        ...
        "])\n"
        "\n"
        "ude=sg('UserDefinedEnum')\n"
        "dtf=sg('DataTableFunctionLibrary')\n"
        "chk('data',[\n"
        "  ('StructureFactory',sg('StructureFactory') is not None),\n"
        "  ('UserDefinedStruct',sg('UserDefinedStruct') is not None),\n"
        "  ('EnumFactory',sg('EnumFactory') is not None),\n"
        "  ('UserDefinedEnum',ude is not None),\n"
        "  ('PBP.enum_num_enums',hasattr(pbp,'enum_num_enums') if pbp else False),\n"
        "  ('PBP.enum_set_meta_data',hasattr(pbp,'enum_set_meta_data') if pbp else False),\n"
        "  ('DataTableFactory',sg('DataTableFactory') is not None),\n"
        "  ('DataTable',sg('DataTable') is not None),\n"
        "  ('DataTableFunctionLibrary',dtf is not None),\n"
        "  ('PBP.data_table_get_row_names',hasattr(pbp,'data_table_get_row_names') if pbp else False),\n"
        "  ('PBP.data_table_fill_from_csv',hasattr(pbp,'data_table_fill_from_csv') if pbp else False),\n"
        "  ('PBP.data_table_to_json_string',hasattr(pbp,'data_table_to_json_string') if pbp else False),\n"
        "  ('PBP.struct_get_fields',hasattr(pbp,'struct_get_fields') if pbp else False),\n"
        "  ('StructureEditorUtils_ABSENT',sg('StructureEditorUtils') is None),\n"
        "])\n"
        "\n"
        "ell=sg('EditorLevelLibrary')\n"
        "chk('scene',[\n"
        ...
        "  ('StaticMeshActor',sg('StaticMeshActor') is not None),\n"
        "  ('PointLight',sg('PointLight') is not None),\n"
        "  ('DirectionalLight',sg('DirectionalLight') is not None),\n"
        "  ('SkyAtmosphere',sg('SkyAtmosphere') is not None),\n"
        "  ('ExponentialHeightFog',sg('ExponentialHeightFog') is not None),\n"
        "])\n"
        "\n"
        "abl=sg('AnimBlueprintEditorLibrary')  # may be None in 5.4\n"
        "chk('animation',[\n"
        "  ('AnimBlueprintFactory',sg('AnimBlueprintFactory') is not None),\n"
        "  ('AnimBlueprint',sg('AnimBlueprint') is not None),\n"
        "  ('PBP.compile_anim_blueprint',hasattr(pbp,'compile_anim_blueprint') if pbp else False),\n"
        "  ('PBP.add_anim_graph_node',hasattr(pbp,'add_anim_graph_node') if pbp else False),\n"
        "  ('AnimBlueprintEditorLibrary_optional',True),\n"
        ...
        "  ('AnimationEditorLibrary',sg('AnimationEditorLibrary') is not None),\n"
        "  ('AnimMontage',sg('AnimMontage') is not None),\n"
        "  ('AnimMontageFactory',sg('AnimMontageFactory') is not None),\n"
        "])\n"
        "\n"
        "chk('umg',[\n"
        "  ('WidgetBlueprintFactory',sg('WidgetBlueprintFactory') is not None),\n"
        "  ('WidgetBlueprint',sg('WidgetBlueprint') is not None),\n"
        "  ('PBP.add_widget_to_designer',hasattr(pbp,'add_widget_to_designer') if pbp else False),\n"
        "  ('SlateFontInfo',sg('SlateFontInfo') is not None),\n"
        "  ('SlateColor',sg('SlateColor') is not None),\n"
        "  ('SlateBrush',sg('SlateBrush') is not None),\n"
        "  ('ButtonStyle',sg('ButtonStyle') is not None),\n"
        "])\n"
        "\n"
        "mel=sg('MaterialEditingLibrary')\n"
        "chk('material',[\n"
        "  ('Material',sg('Material') is not None),\n"
        "  ('MaterialEditingLibrary',mel is not None),\n"
        "  ('PBP.get_material_expressions',hasattr(pbp,'get_material_expressions') if pbp else False),\n"
        ...
        "  ('MaterialInstanceConstant',sg('MaterialInstanceConstant') is not None),\n"
        "  ('MaterialExpressionTextureSample',sg('MaterialExpressionTextureSample') is not None),\n"
        "  ('MaterialExpressionScalarParameter',sg('MaterialExpressionScalarParameter') is not None),\n"
        "])\n"
        "\n"
        "chk('enhanced_input',[\n"
        "  ('InputAction',sg('InputAction') is not None),\n"
        "  ('InputMappingContext',sg('InputMappingContext') is not None),\n"
        "  ('PBP.create_input_action',hasattr(pbp,'create_input_action') if pbp else False),\n"
        "  ('PBP.create_input_mapping_context',hasattr(pbp,'create_input_mapping_context') if pbp else False),\n"
        "  ('EnhancedInputLibrary',sg('EnhancedInputLibrary') is not None),\n"
        "])\n"
        "\n"
        "chk('niagara_seq_bt_pcg',[\n"
        "  ('NiagaraSystem',sg('NiagaraSystem') is not None),\n"
        "  ('NiagaraEmitter',sg('NiagaraEmitter') is not None),\n"
        "  ('LevelSequence',sg('LevelSequence') is not None),\n"
        "  ('LevelSequenceEditorBlueprintLibrary',sg('LevelSequenceEditorBlueprintLibrary') is not None),\n"
        "  ('SequencerTools',sg('SequencerTools') is not None),\n"
        "  ('BehaviorTree',sg('BehaviorTree') is not None),\n"
        "  ('BehaviorTreeFactory',sg('BehaviorTreeFactory') is not None),\n"
        "  ('BlackboardData',sg('BlackboardData') is not None),\n"
        "  ('PBP.create_pcg_graph',hasattr(pbp,'create_pcg_graph') if pbp else False),\n"
        "])\n"
    )
